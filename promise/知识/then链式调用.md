...
2. 新任务的状态取决于**前任务的**后续处理（因为新任务是由前任务通过then或者catch生成的任务）：

   - 若没有相关的后续处理，新任务的状态和前任务一致，数据为前任务的数据

   - 若有后续处理但还未执行，新任务挂起。（前任务还没resolve或者reject时，新任务也是跟前任务一致，也是pending）
   - 若后续处理执行了，则根据后续处理的情况确定新任务的状态
     - 后续处理执行无错，新任务的状态为完成（**前任务是rejected，后任务是catch，那么后任务也是fulfilled！！！**），数据为**后续处理的返回值**（完成了，就不继承前任务的return值了！！！）
     - 后续处理执行**有错**，新任务的状态为**失败（rejected）**（即使是前任务报错，也是直接rejected），数据为异常对象（比如 throw 3 ，那这里就返回 3）
